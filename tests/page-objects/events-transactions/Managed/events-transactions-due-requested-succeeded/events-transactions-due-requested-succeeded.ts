...
export class eventsTransactionsRequest {
  ...
  async validateEventsTransaction() {
    ...
    const merchantConfiguration = await utility.commonJsonReadFunc('jsonFile');
    const eventType = [
      'CustomerPaymentDue',
      ...
      for (let i = 0; i < eventType.length; i++) {
        ...
        if (
          ...
          if (eventType[i] == 'CustomerPaymentSucceeded') {
            await titleArray.push('paymentPlatformVendor', 'paymentGatewayFee');
            await expected.push(
            ...
            await actual.push(
              ...
              CustomerPaymentSucceeded.paymentGatewayFee
            );
          }
          if (x == 0) {
            ...
            case 'CustomerPaymentSucceeded':
              ...
              for (let m = 0; m < allTransactions.length; m++) {
                ...
                const expexctedPaymentGatewayFee =
                  expectedJson.paidPayments[x].paymentGatewayFee;
                const expexctedMerchantTransactionFee =
                  expectedJson.paidPayments[x].MerchantTransactionFee;

                if (
                  allTransactions[m].accountCode ==
                  'Cash Clearing - Customer Deposits (Managed)'
                ) {
                  TotalAmount = allTransactions[m].debitAmount;
                  titleArrayTransctions.push(
                    'Cash Clearing - Customer Deposits (Managed) - Debit Amount'
                  );
                  ...
                  expectedTransctions.push(
                    String(
                      await utility.upto2Decimal(
                        expexctedMerchantTransactionFee
                      )
                    )
                  );
                } else if (
                  allTransactions[m].accountCode ==
                  'PlanPay Sales - Transaction Fees (Managed)'
                ) {
                  TransactionFeeMerchant = allTransactions[m].creditAmount;
                  titleArrayTransctions.push(
                    'PlanPay Sales - Transaction Fees (Managed) - Credit Amount'
                  );
                  actualTransctions.push(
                    await utility.upto2Decimal(TransactionFeeMerchant)
                  );
                ...
                if (
                  allTransactions[m].accountCode ==
                  'PlanPay Cost of Sales - Merchant Fees (Managed)'
                ) {
                  TransactionFeeActual = allTransactions[m].debitAmount;
                  titleArrayTransctions.push(
                    'PlanPay Cost of Sales - Merchant Fees (Managed) - Debit Amount'
                  );
                  actualTransctions.push(
                  ...
                  allTransactions[m].accountCode ==
                  'Cash Clearing - Transaction Fees - Stripe'
                ) {
                  TransactionFeeActual = allTransactions[m].creditAmount;
                  titleArrayTransctions.push(
                    'Cash Clearing - Transaction Fees - Stripe - Credit Amount'
                  );
                  actualTransctions.push(
                    await utility.upto2Decimal(TransactionFeeActual)
                  );
                ...
                console.log('Transctions Title => ', titleArrayTransctions);
                console.log('Actual Transction Values => ', actualTransctions);
                console.log(
                  'Expected Transction Values => ',
                  expectedTransctions
                );
